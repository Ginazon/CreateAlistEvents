  ...
  return (
    <div className="min-h-screen bg-white flex flex-col items-center pb-20 font-sans">
        ...
        <div className="bg-white rounded-xl shadow-xl p-8 border-t-4" style={{ borderColor: themeColor }}>
          ...
          <div className="grid grid-cols-1 gap-4 text-center mb-10">
             ...
             <div className="p-5 bg-gray-50 rounded-xl border border-gray-100">
                ...
                {event.location_url && (
                    <a href={event.location_url} target="_blank" className="inline-block px-6 py-2 rounded-full text-sm font-bold text-white transition hover:opacity-90 shadow-md transform hover:scale-105" style={{ backgroundColor: themeColor }}>Yol Tarifi Al 🗺️</a>
                )}
             </div>
          </div>
          ...
          {detailBlocks.length > 0 && (
            <div className="space-y-8 mb-10">
                <h3 className="text-center font-bold text-gray-400 text-xs uppercase tracking-widest mb-4">Etkinlik Detayları</h3>
      ...
      <div className="max-w-xl w-full px-6 mt-12">
        <h2 className="text-2xl font-bold mb-6 flex items-center gap-3" style={{ color: themeColor }}>📸 Anı Duvarı</h2>
        {currentUserEmail ? (
            ...
            <div className="bg-gray-50 rounded-2xl p-10 text-center border-2 border-dashed border-gray-200">
                ...
                <h3 className="font-bold text-gray-800 text-lg">Galeri Kilitli</h3>
                <p className="text-sm text-gray-500 mt-2">Fotoğrafları görmek ve yüklemek için<br/>yukarıdaki formu doldurarak giriş yapın.</p>
            </div>
        )}
      </div>
      ...
      <div className="max-w-xl w-full px-6 mt-12 pb-10">
          <Link href={homeLink} className="block w-full text-center">
              <button className="bg-gray-100 text-gray-600 px-6 py-3 rounded-full font-bold hover:bg-gray-200 transition text-sm">
                  {isOwner ? "← Dashboard'a Dön" : "Cereget ile kendi davetiyeni oluştur"}
              </button>
          </Link>
      </div>
    </div>
